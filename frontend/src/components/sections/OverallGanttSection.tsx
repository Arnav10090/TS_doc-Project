...
interface OverallGanttSectionProps {
  projectId: string;
}

const OverallGanttSection: React.FC<OverallGanttSectionProps> = ({ projectId }) => {
  ...
  useEffect(() => {
    const loadSection = async () => {
      try {
        ...
        await getSection(projectId, 'overall_gantt');
      } catch (error) {
        console.error('Error loading overall gantt section:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleGeneratePrompt = async () => {
    ...
    try {
      const response = await getAiPrompt(projectId, 'gantt_overall');
    ...
    <div style={{
      ...
      <div style={{ marginBottom: '32px' }}>
        ...
        <p style={{
          ...
          Generate an AI prompt to create your overall project Gantt chart using free tools like
      ...
      <div>
        ...
        <p style={{
          ...
          Upload the generated overall Gantt chart (PNG or JPG, max 10MB).
        </p>
        <DiagramUpload
          projectId={projectId}
          imageType="gantt_overall"
        />
      </div>
