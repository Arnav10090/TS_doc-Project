"""Groq provider implementation for AI Suggestions."""
...
def extract_groq_response_text(payload: Any) -> Optional[str]:
...
class GroqProvider:
    ...
    async def generate(self, request: AIProviderRequest) -> AIProviderResponse:
        ...
        request_timeout = request.timeout_seconds or self._settings.GROQ_TIMEOUT_SECONDS
        messages = []
        ...
        try:
            client = self._client_for_timeout(float(request_timeout))
